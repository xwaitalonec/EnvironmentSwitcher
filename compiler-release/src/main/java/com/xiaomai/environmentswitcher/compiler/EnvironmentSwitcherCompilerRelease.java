...
public class EnvironmentSwitcherCompilerRelease extends EnvironmentSwitcherCompilerDebug {
    ...
    protected FieldSpec generateEnvironmentField(Environment environmentAnnotation,
        ...
        if (environmentAnnotation.isRelease()) {
            return super.generateEnvironmentField(environmentAnnotation, defaultXXEnvironmentFiledBuilder, moduleUpperCaseName, environmentName, environmentUpperCaseName, url, alias, host, imUrl);
        }
        ...
        return FieldSpec.builder(EnvironmentBean.class,
                ...
                .initializer(String.format("new %s(\"%s\", \"%s\", \"%s\", \"%s\", \"%s\", %s%s)",
                        EnvironmentBean.class.getSimpleName(), environmentName, "", alias, imUrl, host, VAR_MODULE_PREFIX, moduleUpperCaseName))
                .build();
    }
}